package project.skripsi.enggyheroedi.utility;

import android.location.Location;
import android.location.LocationManager;

import com.google.android.gms.maps.model.LatLng;
import project.skripsi.enggyheroedi.CityGuideApplication;
...
public class LocationUtility
{
	...
	public static String getDistanceString(double distance, boolean useMetricSystem)
	{
		...
		if(useMetricSystem)
		{
			if(distance<1000.0d)
			{
				result = String.format(Locale.US, "%d " + CityGuideApplication.getContext().getString(R.string.unit_meter), (int) distance);
			}
			else if(distance<5000.0d)
			{
				result = String.format(Locale.US, "%.1f " + CityGuideApplication.getContext().getString(R.string.unit_kilometer), distance/1000);
			}
			else
			{
				result = String.format(Locale.US, "%d " + CityGuideApplication.getContext().getString(R.string.unit_kilometer), (int) distance/1000);
			}
		}
		else
		{
			...
			{
				result = String.format(Locale.US, "%.2f " + CityGuideApplication.getContext().getString(R.string.unit_mile), distanceMiles);
			}
			else if(distanceMiles<10.0d)
			{
				result = String.format(Locale.US, "%.1f " + CityGuideApplication.getContext().getString(R.string.unit_mile), distanceMiles);
			}
			else
			{
				result = String.format(Locale.US, "%d " + CityGuideApplication.getContext().getString(R.string.unit_mile), (int) distanceMiles);
			}
		}
